...
const Docente = () => {
    ...
    const guardarDocente = async (e) => {
        e.preventDefault();
        const isSaved = await create(endopint.docente, {
            nombre,
            apellido,
            tipoDocumento,
            numeroDocumento,
        ...
        if (isSaved) {
            alert("Docente guardada correctamente");
            await getDocentes();
            setNombre('');
        ...
        <>
            <div className='relative z-10 flex-shrink-0 flex h-16 shadow items-center justify-center px-4 bg-white dark:bg-gray-800'>
                <div className="flex justify-center items-center h-screen">
                    <button  onClick={() => setModalIsOpen(true)} className="bg-gradient-to-r from-purple-800 to-purple-500 text-white rounded-lg py-6 px-6 shadow-md hover:shadow-lg">
                        <span className="mr-2">Agregar Docente</span>
                    ...
                    { modalIsOpen &&
                        ...
                        >
                            <h1 className="text-2xl font-semibold text-gray-900">{modoEdicion ? 'Editar' : 'Agregar'} Docente</h1>
                            <form className="flex flex-col w-full mt-6" onSubmit={modoEdicion ? editarDocente : guardarDocente}>
            ...
            { /* Se muestra la tabla de programas académicos */ }
                <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
                    <h1 className="text-2xl font-semibold text-white">Docentes</h1>
                </div>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
                    <div className="py-4">
                            ...
                            {loading ? (
                                ...
                                docentes.map((docente) => (
                                    <tr key={docente.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {docente.programaAcademico.nombre}
                                        </td>

                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <button
                                                onClick={(e) => abrirModoEdicion(
                                                    docente.nombre, docente.uuid, docente.programaAcademico.uuid, docente.apellido,
                                                    docente.tipoDocumento, docente.numeroDocumento, docente.correo
                                                )}
